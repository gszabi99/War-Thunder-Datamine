...
let { convertExpWndOpenParams, closeConvertExpModalWnd } = require("%scripts/convertExp/convertExp.nut")
...
function openConvertExpWnd(unit = null) {
  ...
  loadHandler(ConvertExpHandler, { unit = unit })
}

function closeConvertExpWnd() {
  handlersManager.doDelayed(function() {
    let handler = handlersManager.findHandlerClassInScene(ConvertExpHandler)
    handlersManager.destroyHandler(handler)
    handlersManager.clearInvalidHandlers()
    handler.onModalWndDestroy()
  })
}

convertExpWndOpenParams.subscribe(function(params) {
  if (params == null)
    closeConvertExpWnd()
  if (params != null)
    openConvertExpWnd(params?.unit)
})