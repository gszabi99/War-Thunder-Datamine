    ...
    if (contact)
    {
      ...
      if (needCheckSquadInvitesOnContactsUpdate)
      {
        if (acceptExistingInvite(uid))
          ...
          squadExistCheckArray.update(@(val) v.append(contact.uidInt64))
      }
    }
  }
  notFoundIds.clear()
...
let function checkSquadInvites(xboxIdsList)
{
  ...
  foreach (xboxId in xboxIdsList)
  {
    ...
    if (contact)
    {
      ...
      squadExistCheckArray.update(@(v) v.append(contact.uidInt64))
    }
    else
...
::xbox_update_squad <- @(xboxIdsList) updateSquadList(xboxIdsList).bindenv(this)
