...
local Unit = class
{
   ...
   nvdSights = null // [] NVD sights for NVD mod info
  ...
  function getNVDSights() {
    if (nvdSights)
      return nvdSights

    nvdSights = []
    local nvdBlk = ::get_full_unit_blk(name)?.modifications.night_vision_system.effects.nightVision
    if (!nvdBlk)
      return nvdSights

    eachBlock(nvdBlk, @(_, name) nvdSights.append(name), this)
    return nvdSights
  }
