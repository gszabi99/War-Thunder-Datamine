...
::ContactsHandler <- class extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function reloadSceneData()
  {
    ...
    if (!scene.findObject("contacts_wnd"))
    {
      sceneChanged = true
      this.guiScene = scene.getScene()
      this.guiScene.replaceContent(scene, "%gui/contacts/contacts.blk", this)
      setSavedSizes()
  ...
  onPlayerCancel = @(_obj) ::is_mouse_last_time_used() ? goBack() : selectCurContactGroup()
  ...
  {
    foreach (_idx, groupName in getContactsGroups())
    {
