...
class ::gui_handlers.QueueTable extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function updateTabContent()
  {
    ...
    if (!isClanQueue)
    {
      local listBoxObjItemObj = clustersListBoxObj.childrenCount() > value
        ? clustersListBoxObj.getChild(value)
        : null
      curCluster = listBoxObjItemObj?.id
    }
    local needClusterWarning = isQueueTableVisible && curCluster != null && ::g_clusters.isClusterUnstable(curCluster)
    local unstableClusterWarnObj = scene.findObject("unstable_cluster_warning")
