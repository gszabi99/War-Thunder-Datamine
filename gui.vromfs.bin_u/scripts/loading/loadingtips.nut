...
::g_tips <- {
  ...
  function onEventProfileReceived(p) { this.isTipsValid = false }
}

::g_tips.getTip <- function getTip(unitTypeMask = 0)
{
  if (unitTypeMask != this.curTipUnitTypeMask || this.nextTipTime <= ::dagor.getCurTime())
...
{
  this.nextTipTime = ::dagor.getCurTime() + TIP_LIFE_TIME_MSEC
...
::g_tips.onEventLoginComplete <- function onEventLoginComplete(p) { this.isTipsValid = false }
::g_tips.onEventGameLocalizationChanged <- function onEventGameLocalizationChanged(p) { this.isTipsValid = false }
::g_tips.onEventSignOut <- function onEventSignOut(p) { this.isTipsValid = false }
