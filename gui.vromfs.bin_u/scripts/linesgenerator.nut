from "%globalScripts/guiGeom/linesGenerator.nut" import LinesPriorities, createLinkLines, findGoodPosForBox
...
function getLinkLinesMarkup(config) {
  ...
  let startObjContainer = config?.startObjContainer
  if (!checkObj(startObjContainer))
    return ""

  let endObjContainer = config?.endObjContainer
  if (!checkObj(endObjContainer))
    return ""

  let linksDescription = config?.links
  if (!linksDescription)
    return ""
  ...
  foreach (_idx, linkDescriprion in linksDescription) {
    ...
    startBlock.box.priority = (linkDescriprion.start?.priority ?? LinesPriorities.TEXT)
    ...
    endBlock.box.priority = (linkDescriprion.end?.priority ?? LinesPriorities.TARGET)
  ...
  let lineWidth = (config?.lineWidth ?? "@helpLineWidth")

  let obstacles = config?.obstacles
  if (obstacles != null)
    foreach (_idx, obstacle in obstacles) {
      ...
      obstacleBlock.box.priority = (obstacle?.priority ?? LinesPriorities.OBSTACLE)
