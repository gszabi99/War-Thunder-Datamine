...
let WwBattle = class {
  ...
  getBattleName = @() !this.isValid() ? ""
    : loc("worldWar/battleName", { number = this.getOrdinalNumber() })
  ...
  function getTotalPlayersInQueueInfo(side) {
    ...
    let enemySideNumber = this.getPlayersInQueueBySide(queue, getOppositeSide(side))
  ...
  function getTimeStartAutoBattle() {
    ...
    return (maxBattleWaitTimeSec / (hasOperationTimeOnCreation ? wwGetSpeedupFactor() : 1)).tointeger()
      - ((hasOperationTimeOnCreation ? getOperationTimeSec() : get_charserver_time_sec())
