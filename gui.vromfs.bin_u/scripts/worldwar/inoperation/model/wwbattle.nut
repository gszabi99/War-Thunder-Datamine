...
::WwBattle <- class
{
  ...
  {
    side = side ?? getSide(profileCountrySq.value)
  ...
  function hasAvailableUnits(team = null)
  {
    if (!team)
    {
      let side = getSide(profileCountrySq.value)
      if (side == SIDE_NONE)
        return false
  ...
  function join(side)
  {
    ...
    log("ww: join ww battle op:" + opId.tostring() + ", battle:" + id +
  ...
  {
    if (!hasFeature("worldWarMaster") && !getMyAssignCountry())
  ...
  function getGroupId()
  {
    let playerSide = getSide(profileCountrySq.value)
