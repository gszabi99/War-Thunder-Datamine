...
let DEFAULT_FLAGS = ~(1 << RenderCategory.ERC_ARMY_RADIUSES)
let DEFAULT_PREVIEW_FLAGS = (1 << RenderCategory.ERC_ZONES) | (1 << RenderCategory.ERC_BATTLES) | (1 << RenderCategory.ERC_MAP_PICTURE)
local flags = 0
...
function setCategory(category, enable) {
  ...
  setWWMapFilter(category, enable)
}

function setPreviewCategories() {
  for (local cat = RenderCategory.ERC_ARMY_RADIUSES; cat < RenderCategory.ERC_TOTAL; ++cat) {
