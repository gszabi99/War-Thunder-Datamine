...
::gui_handlers.WwLeaderboard <- class extends ::gui_handlers.LeaderboardWindow
{
  ...
  function fetchLbData(isForce = false)
  {
    ...
    let cb = function(hasSelfRow = false)
    {
      let callback = Callback(
        function(lbPageData) {
          if (!hasSelfRow)
            this.selfRowData = []
          this.pageData = wwLeaderboardData.addClanInfoIfNeedAndConvert(requestData.modeName, lbPageData, isCountriesLeaderboard())
    ...
    if (isUsersLeaderboard() || (this.forClans && ::is_in_clan()))
    {
      let callback = Callback(
        function(lbSelfData) {
          this.selfRowData = wwLeaderboardData.addClanInfoIfNeedAndConvert(requestData.modeName, lbSelfData, isCountriesLeaderboard()).rows
          if(isRequestDifferent)
  ...
  function getRequestData()
  {
    ...
    return {
      ...
      platformFilter = lbModeData?.needShowConsoleFilter ? this.platformFilter : ""
    }
  }
