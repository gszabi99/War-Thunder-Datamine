...
::ActionBar <- class
{
  ...
  function fill()
  {
    ...
    let partails = {
      items           = loadTemplateText("%gui/hud/actionBarItem")
      textShortcut    = canControl ? loadTemplateText("%gui/hud/actionBarItemTextShortcut")    : ""
      gamepadShortcut = canControl ? loadTemplateText("%gui/hud/actionBarItemGamepadShortcut") : ""
    }
  ...
  function buildItemView(item, needShortcuts = false)
  {
    ...
    if (modifName)
    {
      viewItem.bullets <- ::handyman.renderNested(loadTemplateText("%gui/weaponry/bullets"),
        function (_text) {
