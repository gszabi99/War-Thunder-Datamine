...
from "weaponUtils" import NONE_FUSE_MODE, CONTACT_FUSE_MODE, NON_CONTACT_FUSE_MODE, DISTANCE_FUSE_MODE
from "hudActionBar" import getActionShortcutIndexByType, getActionBarUnitName, getOwnerUnitName, getActionDataByType, isHeroHumanDrone
from "hudState" import getHudUnitType
from "blkGetters" import get_current_mission_info_cached
from "radarGuiControls" import AirRadarGuiControlMode, getNextAirRadarGuiControlMode
...
let { EII_BULLET, EII_ARTILLERY_TARGET, EII_ANTI_AIR_TARGET, EII_EXTINGUISHER, EII_SPECIAL_UNIT, EII_WINCH, EII_WINCH_DETACH, EII_WINCH_ATTACH, EII_TOOLKIT, EII_MEDICALKIT, EII_TORPEDO, EII_TORPEDO_SIGHT, EII_HULL_AIMING, EII_DEPTH_CHARGE, EII_MINE, EII_MORTAR, EII_ROCKET, EII_GRENADE, EII_SMOKE_GRENADE, EII_REPAIR_BREACHES, EII_SMOKE_SCREEN, EII_SCOUT, EII_SUBMARINE_SONAR, EII_TORPEDO_SENSOR, EII_SPEED_BOOSTER, EII_SHIP_CURRENT_TRIGGER_GROUP, EII_AI_GUNNERS, EII_AUTO_TURRET, EII_SUPPORT_PLANE, EII_SUPPORT_PLANE_2, EII_SUPPORT_PLANE_3, EII_SUPPORT_PLANE_4, EII_SUPPORT_PLANE_ORBITING, EII_SUPPORT_PLANE_CHANGE, EII_SUPPORT_PLANE_GROUP_FLY_TO, EII_SUPPORT_PLANE_GROUP_ATTACK, EII_SUPPORT_PLANE_GROUP_HUNT, EII_SUPPORT_PLANE_GROUP_DEFEND, EII_SUPPORT_PLANE_GROUP_RETURN, EII_SUPPORT_PLANE_GROUP_ADD_FLY_TO, EII_SUPPORT_PLANE_GROUP_ADD_ATTACK, EII_SUPPORT_PLANE_GROUP_CANCEL, EII_STEALTH, EII_LOCK, EII_GUIDANCE_MODE, EII_FORCED_GUN, EII_AGM_RELEASE_LOCKED_TARGET, WEAPON_PRIMARY, WEAPON_SECONDARY, WEAPON_MACHINEGUN, AI_GUNNERS_DISABLED, AI_GUNNERS_ALL_TARGETS, AI_GUNNERS_AIR_TARGETS, AI_GUNNERS_GROUND_TARGETS, AI_GUNNERS_SHELL, EII_TERRAFORM, EII_DIVING_LOCK, EII_SHIP_DAMAGE_CONTROL, EII_NIGHT_VISION, EII_SIGHT_STABILIZATION, EII_SIGHT_STABILIZATION_OFF, EII_RAGE_SCANNER_ACTION, EII_MULTIPLE_CHOICE_SPECIAL_WEAPON_ACTIVATE, EII_UGV, EII_MINE_DETONATION, EII_UNLIMITED_CONTROL, EII_DESIGNATE_TARGET, EII_HUMAN_WEAPON, EII_HUMAN_MEDKIT, EII_HUMAN_NEXT_SQUADMATE, EII_TOGGLE_VIEW, EII_BURAV, EII_PERISCOPE, EII_RADAR_TARGET_LOCK, EII_SELECT_SPECIAL_WEAPON, EII_MISSION_SUPPORT_PLANE, EII_BUILDING, EII_MANEUVERABILITY_MODE, EII_THRUST_VECTORING_MODE, EII_BOMBER_VIEW, EII_3RD_PERSON_VIEW, EII_BUOYANCY_UP = 88, EII_BUOYANCY_DOWN = 89, EII_SLAVE_UNIT_SPAWN, EII_SLAVE_UNIT_SWITCH, EII_HOVER_MODE, EII_FBW_MODE, EII_MOUSE_AIM_OVERRIDE_ROLL, EII_ANTI_AIR_COMPLEX_MENU, EII_SLAVE_UNIT_STATUS, EII_AIR_RADAR_GUI_CONTROL_MODE, EII_RADAR_SWITCH_TARGET, EII_SENSORS_GROUP_MODE, EII_RADAR_GUI_NAVIGATION, EII_SHIP_SONAR, EII_HUMAN_ORDERS_MENU, EII_SUB_HYDROPHONE, EII_FUSE_MODE, EII_FPV_SHELL_ACTIVATE, EII_SENSOR_MODES_SWITCH, EII_CARRIER_MENU } = require("hudActionBarConst")
...
let { weaponSlotsKeys } = require("%scripts/hud/humanWeaponSlots.nut")

let actionBarIconHeight = @() to_pixels("@hudActionBarItemSize")
...
function getHudKillStreakShortcutId() {
  ...
  if (hudUnitType == HUD_UNIT_TYPE.HUMAN_DRONE)
    return "ID_PLANE_KILLSTREAK_WHEEL_MENU"
...
enumsAddTypes(g_hud_action_bar_type, {
  ...
  SHIP_SONAR = {
    ...
    _name = "ship_sonar"
    _icon = "#ui/gameuiskin#sonar_indicator"
    _title = loc("hotkeys/ID_SHIP_SWITCH_ACTIVE_SONAR")
    getTooltipText = @(actionItem = null) this.getTitle(actionItem)
    getShortcut = @(_actionItem, _hudUnitType = null) "ID_SHIP_SWITCH_ACTIVE_SONAR"
  }
  ...
  SUPPORT_PLANE = {
    ...
    getShortcut = function(_actionItem, hudUnitType = null) {
      ...
      return ((hudUnitType == HUD_UNIT_TYPE.TANK && unit == ownerUnit)
        ...
        : (hudUnitType == HUD_UNIT_TYPE.HUMAN && unit == ownerUnit) ? "ID_START_SUPPORT_PLANE_HUMAN"
        : ((hudUnitType == HUD_UNIT_TYPE.HUMAN_DRONE || hudUnitType == HUD_UNIT_TYPE.HUMAN_DRONE_HELI) && getOwnerUnit()?.isHuman()) ?
          (isHeroHumanDrone() ? "ID_LEAVE_DRONE_CONTROLLING"
    ...
    getName = function(_actionItem, _killStreakTag = null) {
      ...
      return hudUnitType == HUD_UNIT_TYPE.TANK && unit == ownerUnit ? this._name
        : !isAircraft && unit == ownerUnit ? "support_plane_ship"
    ...
    getTitle = function(_actionItem, _killStreakTag = null) {
      ...
      return hudUnitType == HUD_UNIT_TYPE.TANK && unit == ownerUnit ? this._title
        ...
        : !isAircraft && unit == ownerUnit ? loc("hotkeys/ID_START_SUPPORT_PLANE_SHIP")
  ...
  NIGHT_VISION = {
    ...
    function isHidden() {
      let hudUnitType = getHudUnitType()
      return hudUnitType == HUD_UNIT_TYPE.AIRCRAFT
 ...
 AGM_RELEASE_LOCKED_TARGET = {
  ...
  HOVER_MODE = {
    ...
    getIcon = function(actionItem, _killStreakTag = null, _unit = null, hudUnitType = null) {
      hudUnitType = hudUnitType ?? getHudUnitType()
      let vehicle = hudUnitType == HUD_UNIT_TYPE.HELICOPTER ? "heli" : "aircraft"
      let nextState = actionItem.active ? "deactivation" : "activation"
      if (hudUnitType == HUD_UNIT_TYPE.HUMAN_DRONE_HELI || hudUnitType == HUD_UNIT_TYPE.HUMAN_DRONE)
        return "#ui/gameuiskin#quadrotor_hovering"
      return $"#ui/gameuiskin#{vehicle}_vtol_{nextState}"
    }
    getShortcut = function(_actionItem, hudUnitType = null) {
      if (isHeroHumanDrone())
        return "ID_CONTROL_MODE_HUMAN_UAV"
  ...
  HUMAN_ORDERS_MENU = {
    code = EII_HUMAN_ORDERS_MENU
    _name = "human_orders_menu"
    _title = loc("hotkeys/ID_HUMAN_ORDERS_MENU")
    _icon = "#ui/gameuiskin#inf_multi_function_menu"
    getShortcut = @(_actionItem, _hudUnitType = null) "ID_HUMAN_ORDERS_MENU"
  }
  ...
  RADAR_GUI_NAVIGATION = {
    ...
    getShortcut = function(_actionItem, hudUnitType = null){
      ...
      return "ID_TOGGLE_AIR_RADAR_GUI_NAVIGATION"
    }
    getTitle = @(actionItem, _killStreakTag = null) loc($"hotkeys/{this.getShortcut(actionItem)}")
    getTooltipText = @(actionItem = null) this.getTitle(actionItem)
  }
  ...
  HUMAN_WEAPON = {
    ...
    _name = "human_weapon"
    _title = loc("hotkeys/human_weapon")
    _icon = "#ui/gameuiskin#exoskelet_grenade_launcher"
    getIcon = function(actionItem, _killStreakTag = null, _unit = null, _hudUnitType = null) {
      let { shortcutIdx = -1 } = actionItem
      let slotWeaponType = weaponSlotsKeys?[shortcutIdx] ?? ""
      let weaponTemplate = curHeroTemplates.get()?[slotWeaponType] ?? ""

      if (weaponTemplate != "") {
        let width = actionBarIconHeight()
        return icon3dByGameTemplate(weaponTemplate, {
          width
          height = width
      ...
      if (shortcutIdx == 0)
        return "#ui/gameuiskin#machinegun"
      if (shortcutIdx == 1)
        return "#ui/gameuiskin#pistol"
      if (shortcutIdx == 2)
        return "#ui/gameuiskin#bazooka"
      if (shortcutIdx == 3)
        return "#ui/gameuiskin#knife"
      if (shortcutIdx == 4)
        return "#ui/gameuiskin#grenade"

      return "#ui/gameuiskin#exoskelet_grenade_launcher"
    }
    getShortcut = function(actionItem = null, _hudUnitType = null) {
      ...
      if (shortcutIdx == 0)
        return "ID_HUMAN_MAIN_WEAPON"
      if (shortcutIdx == 1)
        return "ID_HUMAN_PISTOL"
      if (shortcutIdx == 2)
        return "ID_HUMAN_SPECIAL_1"
      if (shortcutIdx == 3)
        return "ID_HUMAN_MELEE"
      if (shortcutIdx == 4)
        return "ID_HUMAN_GRENADE_NEXT"
      return "ID_HUMAN_NEXT_SLOT"
    }
    getTooltipText = @(actionItem = null) actionItem?.weaponName
  }

  HUMAN_MEDKIT = {
    code = EII_HUMAN_MEDKIT
    _name = "human_medkit"
    _title = loc("hotkeys/human_medkit")
    _icon = "#ui/gameuiskin#first_aid_kit"
    getShortcut = @(_actionItem, _hudUnitType = null) "ID_HUMAN_USE_MEDKIT"
  }

  HUMAN_NEXT_SQUADMATE = {
    code = EII_HUMAN_NEXT_SQUADMATE
    _name = "human_squad"
    _title = loc("hotkeys/human_next_squadmate")
    _icon = "#ui/gameuiskin#inf_change"
    getShortcut = @(_actionItem, _hudUnitType = null) "ID_CREW_SWITCH_CONTROL_NEXT"
  }
})
