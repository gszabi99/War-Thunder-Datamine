...
function reset(safe = false) {
  if (safe && battleLog.len() && battleLog[battleLog.len() - 1].time < get_mission_time())
    return
  battleLog.clear()
...
let rePatternNumeric = regexp2("^\\d+$")
...
function getActionTextIconic(msg) {
  ...
  if (msgAction == "kill")
    iconId = "".concat(iconId, getUnitTypeSuffix(getUnitTypeEx(msg, false)))
  if (msgAction == "kill" || msgAction == "crash")
    iconId = "".concat(iconId, getUnitTypeSuffix(getUnitTypeEx(msg, true)))
...
function getUnitNameEx(playerId, unitNameLoc = "", teamId = 0, player = null, withPlayerName = true) {
  ...
  return buildPlayerUnitName(player, unitNameLoc)
}
...
function msgMultiplayerDmgToText(msg, iconic = false, player = null, victimPlayer = null) {
  ...
  let who = getUnitNameEx(msg?.playerId ?? userIdInt64.get(), msg?.unitNameLoc ?? userName.get(), msg?.team ?? Team.A, player)
  let whom = getUnitNameEx(msg?.victimPlayerId ?? userIdInt64.get(),
...
function onHudMessage(msg) {
  ...
  if (msg.type == HUD_MSG_MULTIPLAYER_DMG) { 
    ...
    let text = msgMultiplayerDmgToText(msg, false, p1, p2)
