...
{
  let row = [{ id = textId, text = "#" + config.name, tdalign = "left" }]
  ...
  let s = config.timeFormat? time.hoursToString(time.secondsToHours(value), false) : value
  let tooltip = diff.getLocName()
