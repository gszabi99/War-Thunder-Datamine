...
let class EveryDayLoginAward extends ::gui_handlers.BaseGuiHandlerWT {
  ...
  function updateButtons()
  {
    ...
    if (isOpened)
      this.scene.findObject("btn_nav_open").setValue(rouletteAnimationFinished || useSingleAnimation
        ? loc("mainmenu/btnClose")
        : loc("msgbox/btn_skip"))

    ::show_facebook_screenshot_button(this.scene, isOpened && rouletteAnimationFinished)
    updateExpTexts()
  }
  ...
  function updateExpTexts() {
    ...
    updateTodayLoginExp(this.scene.findObject("today_login_exp"), todayLoginExp.value)
    updateLoginStreakExp(this.scene.findObject("login_streak_exp"), loginStreak.value)
  }
