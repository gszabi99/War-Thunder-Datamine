...
::SessionLobby <- {
  ...
  function getLockedCountryData() {
    ...
    let availableCountries = this.getTeamData(crsSetTeamTo)?.countries ?? []
...
::SessionLobby.prepareSettings <- function prepareSettings(missionSettings)
{
  ...
  _settings.chatPassword <- isInRoom() ? this.getChatRoomPassword() : ::gen_rnd_password(16)
...
::SessionLobby.setSettings <- function setSettings(v_settings, notify = false, checkEqual = true)
{
  ...
  this.roomUpdated = notify || !this.isRoomOwner || !isInRoom() || this.isEventRoom
  if (!this.roomUpdated)
...
::SessionLobby.checkDynamicSettings <- function checkDynamicSettings(silent = false, v_settings = null)
{
  ...
  if (changed && !silent)
    setSettings(this.settings, false, false)
}
...
{
  return ::events.getEventUnitTypesMask(this.getMGameMode(room) || getPublicData(room))
}

::SessionLobby.getRequiredUnitTypesMask <- function getRequiredUnitTypesMask(room = null)
{
  return ::events.getEventRequiredUnitTypesMask(this.getMGameMode(room) || getPublicData(room))
}
...
{
  return this.getDifficulty(room).getEdiffByUnitMask(getUnitTypesMask(room))
}
...
::SessionLobby.isMissionReady <- function isMissionReady()
{
  return !isUserMission() ||
         (this.status != lobbyStates.UPLOAD_CONTENT && this.uploadedMissionId == getMissionName())
}

::SessionLobby.uploadUserMission <- function uploadUserMission(afterDoneFunc = null)
{
  if (!isInRoom() || !isUserMission() || this.status == lobbyStates.UPLOAD_CONTENT)
    return
  if (this.uploadedMissionId == getMissionName()) {
    afterDoneFunc?()
    return
  }
...
::SessionLobby.checkUpdateMatchingSlots <- function checkUpdateMatchingSlots()
{
  if (this.hasSessionInLobby())
  {
    ...
    setReady(this.isReady, true, true)
}
...
::SessionLobby.checkMyTeam <- function checkMyTeam() //returns changed data
{
  ...
  if (setTeamTo != Team.none && this.setTeam(setTeamTo, true))
  {
    data.team <- this.team
    let myCountry = profileCountrySq.value
    let availableCountries = getTeamData(this.team)?.countries ?? []
...
::SessionLobby.setTeam <- function setTeam(newTeam, silent = false) //return is team changed
{
  ...
  if (!silent)
    syncMyInfo({team = this.team}, true)
...
{
  if (!canBeSpectator() && !this.spectator)
    return false
...
::SessionLobby.setSpectator <- function setSpectator(newSpectator) //return is spectator changed
{
  ...
  syncMyInfo({spectator=this.spectator}, true)
  return true
}
...
{
  return !isInRoom() || !this.isEventRoom
}
...
{
  if (this.roomId == v_roomId && isInRoom())
    return

  if (!::g_login.isLoggedIn() || isInRoom())
  {
    this.delayedJoinRoomFunc = (@(v_roomId, senderId, v_password, cb) function() { joinRoom(v_roomId, senderId, v_password, cb) })(v_roomId, senderId, v_password, cb)
...
::SessionLobby.afterRoomJoining <- function afterRoomJoining(params)
{
  ...
  if (!this.isRoomOwner || ::u.isEmpty(this.settings))
  {
    ...
    if (mGameMode)
    {
      setIngamePresence(public, this.roomId)
    ...
    if (this.isRoomByQueue && !this.isSessionStartedInRoom())
      this.isRoomByQueue = false
    if (this.isEventRoom && !this.isRoomByQueue && haveLobby())
  ...
  for(local i = this.members.len()-1; i>=0; i--)
    ...
    {
      updateMemberHostParams(this.members[i])
  ...
  setRoomInSession(this.isSessionStartedInRoom())
...
{
  if (!("memberId" in member) || !this.isRoomOwner || !isInRoom())
    return
...
::SessionLobby.getMembersCountByTeams <- function getMembersCountByTeams(room = null, needReadyOnly = false)
{
  ...
  foreach(m in roomMembers)
  {
    ...
    if (needReadyOnly)
      if (!this.hasSessionInLobby() && !isMemberReady(m))
        continue
      else if (this.hasSessionInLobby() && !isMemberInSession(m))
        continue
...
{
  if (this.isRoomOwner && !this.isRoomByQueue && !haveLobby() && this.roomUpdated
...
::SessionLobby.startSession <- function startSession()
{
  ...
  ::room_start_session({ roomId = this.roomId, cluster = getPublicParam("cluster", "EU") },
      function(p)
      {
...
{
  return isInRoom() && !::is_my_userid(uid) && haveLobby() && !this.isPlayerInMyRoom(uid)
}
...
{
  return isInRoom() && (this.isRoomOwner || (haveLobby() && !this.isRoomByQueue))
}
...
::SessionLobby.onEventSquadStatusChanged <- @(_p) checkSquadAutoInvite()
...
::SessionLobby.checkSessionInvite <- function checkSessionInvite()
{
  ...
  let inviteData = this.reconnectData.inviteData
  let sendResp = this.reconnectData.sendResp
  ...
  this.reconnectData.sendResp = null
}
