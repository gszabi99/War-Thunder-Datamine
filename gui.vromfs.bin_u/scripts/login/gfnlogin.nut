...
gui_handlers.LoginWndHandlerGfn <- class (gui_handlers.LoginWndHandler) {
  ...
  function doLogin() {
    log("GFN login: checkLoginPass")
    statsd.send_counter("sq.game_start.request_login", 1, { login_type = "gfn" })
    addLoginState(LOGIN_STATE.LOGIN_STARTED)
    let ret = checkLoginPass("", "", "gfn", "gfn", false, false)
