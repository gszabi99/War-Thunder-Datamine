...
local { getModificationName, getModificationInfo } = require("scripts/weaponry/bulletsVisual.nut")
...
local function appendOneBulletsItem(descr, modifName, air, amountText, genTexts, enabled = true)
{
  ...
  local bulletsSet = getBulletsSetData(air, modifName)
  item.text    <- " ".join([ amountText, getModificationName(air, modifName, bulletsSet) ])
  item.tooltip <- " ".join([ amountText, getModificationInfo(air, modifName, bulletsSet).desc ])
}
