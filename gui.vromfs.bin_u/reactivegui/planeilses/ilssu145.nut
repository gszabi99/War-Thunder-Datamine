...
let {IlsColor, IlsLineScale, BombCCIPMode, RocketMode, BombingMode,
 CannonMode, TargetPosValid, TargetPos, IlsPosSize} = require("%rGui/planeState/planeToolsState.nut")
let {baseLineWidth, mpsToKnots, metrToFeet, GuidanceLockResult} = require("ilsConstants.nut")
let {flyDirection} = require("commonElements.nut")
let {floor, abs, sin} = require("%sqstd/math.nut")
...
let groundReticle = @() {
  watch = [isAAMMode, BombingMode, TargetPosValid]
  size = flex()
  children = !isAAMMode.value && !BombingMode.value && TargetPosValid ? [
    @(){
...
let shellName = @() {
  watch = [IlsColor, CurWeaponName, RocketMode, CannonMode, isAAMMode, BombCCIPMode, BombingMode]
  size = SIZE_TO_CONTENT
  rendObj = ROBJ_TEXT
  ...
  fontSize = 45
  font = Fonts.hud
  text = BombCCIPMode.value ? "CCIP" : (BombingMode.value ? "AUTO" : (RocketMode.value ? "RKT" : (CannonMode.value ? "GUN" : (isAAMMode.value ? ::loc(CurWeaponName.value) :  "GUN"))))
}

let shellCount = @() {
  watch = [IlsColor, ShellCnt, GunMode, BombMode]
  size = SIZE_TO_CONTENT
  rendObj = ROBJ_TEXT
  pos = [pw(80), ph(80)]
  color = IlsColor.value
  fontSize = 45
  font = Fonts.hud
  text = BombMode.value ? "" : GunMode.value ? string.format("%03d", ShellCnt.value) : ShellCnt.value.tointeger()
}

let flyDirHide = Computed(@() abs(TargetPos.value[0] - IlsPosSize[2] * 0.5) < IlsPosSize[2] * 0.05 && abs(TargetPos.value[1] - IlsPosSize[3] * 0.5) < IlsPosSize[3] * 0.05)
  ...
  return @(){
    ...
    [
      @(){
        watch = flyDirHide
        size = flex()
        children = !flyDirHide.value ? [flyDirection(width, height, true)] : null
      }
    ]
  }
}
