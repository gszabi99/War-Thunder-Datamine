...
let { CurWeaponName, GunBullets0, GunBullets1 } = require("%rGui/planeState/planeWeaponState.nut")
...
let CalcFlightTime = Computed(@() round(AamTimeOfFlightMax.value).tointeger())
...
let function AamReady(is_left) {
  return @() {
    ...
    children = [
      ...
      {
        rendObj = ROBJ_VECTOR_CANVAS
        pos = [is_left ? pw(33.8) : pw(60.7), ph(89)]
        size = [pw(5), ph(5)]
        color = IlsColor.value
        lineWidth = IlsLineScale.value * baseLineWidth
        fillColor = Color(0, 0, 0, 0)
...
let gunBullets = @() {
  ...
  [
    gunBulletsCnt(true, GunBullets0),
    gunBulletsCnt(false, GunBullets1)
  ] : null
}
