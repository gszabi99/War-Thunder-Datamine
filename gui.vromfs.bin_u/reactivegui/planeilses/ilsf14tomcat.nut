...
function targetMark(width, height, watchVar, is_radar) {
  return @() {
    ...
    update = function() {
      ...
      return {
        transform = {
          translate = watchVar.value ? (is_radar ? RadarTargetPosLim : TargetPos.get()) : [0, 0]
        }
      }
    }
  }
