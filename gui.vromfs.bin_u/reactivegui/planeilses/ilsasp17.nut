let {IlsColor, TargetPosValid, TargetPos, IlsLineScale, DistToTarget} = require("reactiveGui/planeState/planeToolsState.nut")
...
let function ASP17(width, height) {
  return {
    ...
    update = @() {
      transform = {
        translate = TargetPosValid.value ? [TargetPos.value[0] - width * 0.5, TargetPos.value[1] - height * 0.5] : [0, 0]
      }
    }
  }
}
