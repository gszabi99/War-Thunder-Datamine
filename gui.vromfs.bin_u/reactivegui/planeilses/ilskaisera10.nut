...
let a10BarAltValue = Computed(@() clamp(BarAltitude.get() * metrToFeet * 0.1, -200, 3800).tointeger())
let a10BarAltitude = @() {
  ...
  fontSize = 40
  font = Fonts.hud
  text = $"{a10BarAltValue.get()}0"
}

let a10TangageValue = Computed(@() round(Tangage.get()))
let a10Tangage = @() {
  ...
  fontSize = 40
  font = Fonts.hud
  text = $"{a10TangageValue.get()}"
}

let a10AltValue = Computed(@() clamp(Altitude.get() * metrToFeet * 0.1, 0, 500).tointeger())
let a10Altitude = @() {
...
function pitch(width, height, generateFunc) {
  ...
  return {
    ...
    update = function() {
      let angle = 90.0 - lerp(0.0, 1.0, Tangage.get(), TVVPitch.get(), getTVVLerpT())
      return {
        transform = {
...
let maverickDist = Computed(@() (AtgmTargetDist.get() < 0 ? -1 : AtgmTargetDist.get() * metrToNavMile * 10.0).tointeger())
let maverickAimMark = @() {
  ...
  commands = [
    ...
    [VECTOR_LINE, 0, 0, 0, 0],
    (IlsAtgmLocked.get() ? [VECTOR_LINE, -100, 0, -50, 0] : []),
    (IlsAtgmLocked.get() ? [VECTOR_LINE, 50, 0, 100, 0] : []),
    (IlsAtgmLocked.get() ? [VECTOR_LINE, 0, -50, 0, -100] : []),
    (IlsAtgmLocked.get() ? [VECTOR_LINE, 0, 50, 0, 100] : [])
  ]
  children = [
    @() {
      ...
      fontSize = 30
      hplace = ALIGN_CENTER
      text = maverickDist.get() < 0 ? "" : string.format("%.1f", maverickDist.get() * 0.1)
    }
  ]
  behavior = Behaviors.RtPropUpdate
  update = @() {
...
let ccipDistF = Computed(@() BombingMode.get() ? cvt(TimeBeforeBombRelease.get(), 0, 15, -90, 270).tointeger() :
 cvt(clamp(DistToTarget.get() * metrToFeet * 0.01, 0, 120), 0, 120, -90, 270).tointeger())
let ccipDistM = Computed(@() (DistToTarget.get() * metrToNavMile * 10.0).tointeger())
...
let gunAimMark = @() {
    ...
    @() {
      ...
      children = !isDistanceValid.get() ? null :[
        @() {
          ...
          fontSize = 30
          hplace = ALIGN_CENTER
          text = string.format("%.1f", ccipDistM.get() * 0.1)
        },
        @() {
          watch = ccipDistF
          size = flex()
          ...
          lineWidth = baseLineWidth * IlsLineScale.get()
          commands = [
            ([VECTOR_SECTOR, 0, 0, 90, 90, -90, ccipDistF.get()]),
  ...
  update = @() {
    transform = {
      translate = BombingMode.get() ? [IlsPosSize[2] * 0.5, IlsPosSize[3] * 0.5] : [TargetPos.get()[0], TargetPos.get()[1]]
    }
  }
}
...
function modeTxt(c_version) {
  return @() {
    ...
    text = GuidanceLockState.get() <= GuidanceLockResult.RESULT_STANDBY ?
    (BombingMode.get() ? "CCRP" : (RocketMode.get() || BombCCIPMode.get() ? "CCIP" : (CannonMode.get() ? "GUNS" : ""))) : "AIR-TO-AIR"
  }
}
...
function shellCntText(c_version) {
  return @() {
    ...
    fontSize = 30
    font = Fonts.hud
    text = CannonMode.get() || AirCannonMode.get() ? string.format(c_version ? (AirCannonMode.get() ? "MIG-29/%d" : "CM/%d") : "HEI/%d", ShellCnt.get()) : ""
    halign = ALIGN_RIGHT
  }
}
...
let secondaryWeaponName = @(){
  ...
  children = !CannonMode.get() && !AirCannonMode.get() && CurWeaponName.get() != "" ? @(){
    ...
    font = Fonts.hud
    fontSize  = 35
    text = loc_checked(string.format("%s/a_10c", CurWeaponName.get()))
    halign = ALIGN_RIGHT
  } : null
}
