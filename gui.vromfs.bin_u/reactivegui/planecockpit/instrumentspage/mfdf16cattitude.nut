...
from "%rGui/planeState/planeToolsState.nut" import RadarTargetPosValid, RadarTargetDist, RocketMode, CannonMode, BombCCIPMode, BombingMode, DistToTarget
from "%rGui/planeIlses/ilsConstants.nut" import baseLineWidth, mpsToKnots, metrToFeet, feetToNavMile
from "%rGui/planeState/planeWeaponState.nut" import ShellCnt
...
const mfdWhite = Color(255, 255, 255, 240)
const mfdLineWidth = 0.5
...
let ilsMode = @() {
  watch = [isAAMMode, CCIPMode, BombingMode, ShellCnt, CannonMode, isDGFTMode]
  size = SIZE_TO_CONTENT
  rendObj = ROBJ_TEXT
...
function pitchHorizon(width, height){
  const groundColor = Color(143, 62, 0)
  const skyColor = Color(0, 13, 201)

  const minVisibleFraction = 0.01
