from "%rGui/airState.nut" import FlaresCount, ChaffsCount, RocketsCount, RocketsSalvo, IsLaserDesignatorEnabled
from "%rGui/planeState/planeWeaponState.nut" import WeaponSlots, WeaponSlotsTrigger, WeaponSlotsCnt, SelectedTrigger, SelectedWeapSlot, WeaponSlotsTotalCnt, LaunchImpossible, WeaponSlotsGuidanceType
from "%rGui/planeIlses/ilsConstants.nut" import weaponTriggerName
from "%rGui/planeState/planeToolsState.nut" import BlkFileName
from "guidanceConstants" import GuidanceType
from "%rGui/globals/ui_library.nut" import *

let { CannonCount, CannonMode } = require("%rGui/airState.nut")
const baseColor = Color(0, 255, 0, 255)
  ...
  children = [
    ...
    (SelectedTrigger.get() == weaponTriggerName.AGM_TRIGGER ? {
      ...
      children = {
        ...
        color = baseColor
        pos = const [-3, 0]
        size = const [baseFontSize, baseFontSize * 1.1]
      }
    } : null),
    (SelectedTrigger.get() == weaponTriggerName.AGM_TRIGGER ? {
      size = SIZE_TO_CONTENT
      pos = const [pw(1), ph(29)]
      rendObj = ROBJ_TEXT
      color = baseColor
      font = Fonts.ah64
      fontSize = baseFontSize
    ...
    (SelectedTrigger.get() == weaponTriggerName.AGM_TRIGGER ? {
      ...
      children = {
        rendObj = ROBJ_FRAME
        color = baseColor
        size = const [FLEX, baseFontSize * 1.1]
      }
    } : null),
    (SelectedTrigger.get() == weaponTriggerName.AGM_TRIGGER ? {
      size = SIZE_TO_CONTENT
      ...
      if (WeaponSlotsTrigger.get()[i] == weaponTriggerName.AGM_TRIGGER && WeaponSlotsTotalCnt.get()[i] <= 4) {
        ...
        for (local j = 0; j < 4; ++j) {
          ...
          rocketsSymbol.append(@(){
            ...
            fillColor = SelectedTrigger.get() == weaponTriggerName.AGM_TRIGGER && !isNext ? baseColor : Color(0, 0, 0, 255)
            lineWidth = baseLineWidth
            commands = [
