  ...
  for (local i = 0; i < GearCount.get(); ++i) {
    childrens.append({
      ...
      children = [
        ...
        {
          size = static [pw(30), SIZE_TO_CONTENT]
          halign = ALIGN_RIGHT
          rendObj = ROBJ_TEXT
          color = baseColor
          fontSize = baseFontSize
...
let slidingVal = @(watched_val) function() {
  return {
    size = static [pw(28), ph(100)]
    children = [
      @(){
        pos = [pw(15), 0]
        size = static [pw(70), ph(100)]
        watch = watched_val
        rendObj = ROBJ_SOLID
