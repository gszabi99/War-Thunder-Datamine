...
let victimHitEid = Computed(@() currentHitMark.get()?.victimEid ?? ecs.INVALID_ENTITY_ID)


function updateHitMarks(hitMarksRes) {
  let res = hitMarksRes.filter(@(v) v?.hitPos == null || !v?.isMelee)
  currentHitMark.set(res?.len() ? res.top() : null)
}
...
function mkScreenHitMark() {
  return {
    ...
    children = {
      data = {
        eid = victimHitEid.get()
        minDistance = 0.0
      }
      transform = victimHitEid.get() == ecs.INVALID_ENTITY_ID ? null : {}
      valign = ALIGN_CENTER
      halign = ALIGN_CENTER
      children = [
        hitMarkNormal
