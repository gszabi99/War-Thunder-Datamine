...
local function createParam(param, width, height, isBackground, needCaption = true, for_ils = false, isBomberView = false) {
  ...
  local captionComponent = @() styleLineForeground.__merge({
    rendObj = ROBJ_DTEXT
  ...
  local selectedComponent = @() styleLineForeground.__merge({
    rendObj = ROBJ_DTEXT
  ...
  local valueComponent = @() styleLineForeground.__merge({
  ...
  local additionalComponent = @() styleLineForeground.__merge({
  ...
  local function getChildren(isBackground, isBomberView = false) {
    ...
    foreach(key, param in textParamsMapMain) {
      ...
      if (key == AirParamsMain.ALTITUDE && is_aircraft) {
        children.append(@() styleLineForeground.__merge({
          rendObj = ROBJ_DTEXT
          size = [0, hdpx(12)]
          text = @() ""
        }))
...
local function launchDistanceMaxComponent(colorWatch, width, height, posX, posY, isBackground) {
  ...
  local launchDistanceMax = @() styleLineForeground.__merge({
    rendObj = ROBJ_DTEXT
    halign = ALIGN_CENTER
    valign = ALIGN_CENTER
    color = colorWatch.value
...
local function rangeFinderComponent(colorWatch, posX, posY, isBackground) {
  local rangefinder = @() styleLineForeground.__merge({
    rendObj = ROBJ_DTEXT
    halign = ALIGN_CENTER
...
local detectAllyComponent = @(posX, posY, isBackground) function(){
  ...
  return res.__update(styleLineForeground.__merge({
    rendObj = ROBJ_DTEXT
