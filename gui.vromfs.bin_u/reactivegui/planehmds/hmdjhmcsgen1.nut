...
from "%rGui/planeState/planeFlyState.nut" import Speed, BarAltitude, Overload, CompassValue
from "%rGui/planeIlses/ilsConstants.nut" import mpsToKnots, metrToFeet, metrToNavMile, mpsToKmh
from "%rGui/style/airHudStyle.nut" import hudFontHgt
from "%rGui/twsState.nut" import IsTwsActivated, rwrTargetsTriggers
from "%rGui/planeRwrs/rwrAnAlr56Components.nut" import settings
from "%rGui/planeState/planeToolsState.nut" import HmdYaw, RadarTargetPosValid, RadarTargetDistRate
from "%rGui/rocketAamAimState.nut" import TrackerVisible, TrackerX, TrackerY, GuidanceLockState
from "%rGui/airState.nut" import TATargetVisible
from "%rGui/hud/targetTrackerState.nut" import TargetX, TargetY
from "%rGui/style/screenState.nut" import isInVr
from "%rGui/radarState.nut" import DistanceMax, AamLaunchZoneDistMax, AamLaunchZoneDistMin, AamLaunchZoneDistDgftMin, AamLaunchZoneDistDgftMax, AamLaunchZoneDist
...
let rwrPriorityTarget = Computed(function() {
  rwrTargetsTriggers.get()
  for (local i = 0; i < rwrTargets.len(); i++)
    if (rwrTargets[i]?.valid)
      return rwrTargets[i]
  return null
})

let rwrTargetText = Computed(@() rwrPriorityTarget.get() != null
  ? (settings.get().directionGroups?[rwrPriorityTarget.get().groupId].text ?? settings.get().unknownText)
  : settings.get().unknownText)

let rwrTargetAngle = Computed(@() rwrPriorityTarget.get() != null
  ? atan2(rwrPriorityTarget.get().y, rwrPriorityTarget.get().x) * (180.0 / PI) + 90
  : null)

let rwr = @() {
  watch = IsTwsActivated
  size = ph(7)
  pos = const [pw(38), ph(35)]
  children = IsTwsActivated.get() ? [
    ...
    @() {
      ...
      valign = ALIGN_CENTER
      fontSize = hudFontHgt * 1.2
      text = rwrTargetText.get()
    }
    @() {
      watch = rwrTargetAngle
      size = rwrTargetAngle.get() != null ? FLEX : [0, 0]
      rendObj = ROBJ_VECTOR_CANVAS
      color = baseColor
      lineWidth = baseLineWidth
      fillColor = Color(0,0,0,0)
      commands = rwrTargetAngle.get() != null
        ? [[VECTOR_POLY, 42.5, 7.5, 50, 0, 57.5, 7.5, 50, 15]]
        : []
      transform = {
        pivot = [0.5, 0.5]
        rotate = rwrTargetAngle.get() ?? 0
      }
    }
  ] : null
}
