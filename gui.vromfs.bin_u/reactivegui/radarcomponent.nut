...
let {hudFontHgt, fontOutlineFxFactor, greenColor, fontOutlineColor,
  isColorOrWhite, isDarkColor} = require("style/airHudStyle.nut")
...
let makeRadarModeText = @(textConfig, color) function() {
  return styleText.__merge({
    ...
    color
    fontFxFactor = isDarkColor(color) ? 0 : fontOutlineFxFactor
  }).__merge(textConfig)
}
...
let mkRadarTgtsDist = @(dist, _id, width, color) styleText.__merge({
  ...
  pos = [width + hdpx(5), 0]
  fontSize = hudFontHgt
  fontFxFactor = isDarkColor(color) ? 0 : min(hdpx(8), 8)
