...
local radarHdpx = @(px) MfdRadarEnabled.value ? (px * radarPosSize.h / 1024) : hdpx(px)
...
local noiseSignal = function(size, pos1, pos2) {
  local getChildren = function() {
    if (!IsNoiseSignaVisible.value)
      return null
    local children = []
    if (IsRadarVisible.value && NoiseSignal.value > 0.5)
      children.append(@(){
        size = size
        pos = pos1
        children = noiseSignalComponent(NoiseSignal, size, true)
      })
    if (IsRadar2Visible.value && NoiseSignal2.value > 0.5)
      children.append(@(){
        size = size
        pos = pos2
        children = noiseSignalComponent(NoiseSignal2, size, false)
    })
    return children
  }
  ...
  return @(){
    size = SIZE_TO_CONTENT
    children = !MfdRadarEnabled.value ? getChildren() : []
    watch = [
      IsRadarVisible,
      IsRadar2Visible,
      NoiseSignal,
...
local function makeRadarModeText () {
  local texts = []
  if (RadarModeNameId.value >= 0)
    texts.append(::loc(modeNames[RadarModeNameId.value]))
  else if (IsRadarVisible.value)
    texts.append(Irst.value ? ::loc("hud/irst") : ::loc("hud/radarEmitting"))
  return "".join(texts)
}

local function makeRadar2ModeText() {
  local texts = []
  if (Radar2ModeNameId.value >= 0)
    texts.append(::loc(modeNames[Radar2ModeNameId.value]))
  else if (IsRadar2Visible.value)
    texts.append(Irst.value ? ::loc("hud/irst") : ::loc("hud/radarEmitting"))
  return "".join(texts)
}

local B_ScopeSquareMarkers = function(radarWidth, radarHeight, is_mfd) {
  local isCollapsed = (!is_mfd && !IsRadarVisible.value)
  local hiddenText = (isCollapsed && !IndicationForCollapsedRadar.value)
  if (!hiddenText){
    local offsetScaleFactor = 1.3
    local fontSize = (!is_mfd && IsRadarVisible.value) ? getFontSize(is_mfd) : hdpx(13)
    return {
      ...
      children = [
        {
          ...
          children = @() styleText.__merge({
            rendObj = ROBJ_DTEXT
            size = SIZE_TO_CONTENT
            pos = is_mfd ? [ radarWidth * 0.2, -radarHdpx(40)] : [hdpx(4), hdpx(4)]
            hplace = ALIGN_RIGHT
        ...
        @() styleText.__merge({
          rendObj = ROBJ_DTEXT
          size = SIZE_TO_CONTENT
          pos = is_mfd ? [ radarWidth * 0.8, -radarHdpx(40)] : [radarWidth + hdpx(4), hdpx(4)]
          watch = [ HasDistanceScale, DistanceMax, DistanceScalesMax ]
          text = HasDistanceScale.value
            ? ::str(::cross_call.measureTypes.DISTANCE.getMeasureUnitsText(DistanceMax.value * 1000.0),
        ...
        styleText.__merge({
          rendObj = ROBJ_DTEXT
          size = SIZE_TO_CONTENT
          pos = is_mfd ? [ radarWidth * 0.8, radarHeight + radarHdpx(10)] :
            [radarWidth + hdpx(4), radarHeight - hdpx(20)]
          text = !isCollapsed ? ::cross_call.measureTypes.DISTANCE.getMeasureUnitsText(0.0) : null
          fontSize = fontSize
        })
        @() styleText.__merge({
          rendObj = ROBJ_DTEXT
        ...
        styleText.__merge({
          ...
          text = !isCollapsed ? makeRadarModeText() : null
          fontSize = fontSize
        })
        styleText.__merge({
          rendObj = ROBJ_DTEXT
          ...
          text = !isCollapsed ? makeRadar2ModeText() : null
          fontSize = fontSize
        })
        noiseSignal(
...
local B_ScopeSquare = function(width, height, is_mfd) {
  local getChildren = function() {
    local children = [
      B_ScopeSquareBackground(width, height),
      B_ScopeSquareTargetSectorComponent(width, TurretAzimuth, TargetRadarDist, TargetRadarAzimuthWidth, height, targetSectorColor),
      B_ScopeSquareAzimuthComponent(width, height, TurretAzimuth, null, null, true),
      {
        size = [width, height]
    ...
    if (IsRadarVisible.value)
      children.append(B_ScopeSquareAzimuthComponent(width, height, Azimuth, Distance, AzimuthHalfWidth, false))
    if (IsRadar2Visible.value)
      children.append(B_ScopeSquareAzimuthComponent(width, height, Azimuth2, Distance2, AzimuthHalfWidth2, false))
    if (IsAamLaunchZoneVisible.value && HasDistanceScale.value)
      children.append(B_ScopeSquareLaunchRangeComponent(width, height, AamLaunchZoneDist,
  ...
  return @() {
    ...
    children = [
      ...
      B_ScopeSquareMarkers(width, height, is_mfd)
    ]
  }
}
...
local B_ScopeCircleMarkers = function(radarWidth, radarHeight) {
  local hiddenText = (!IsRadarVisible.value && !IndicationForCollapsedRadar.value)
  if (!hiddenText){
    local offsetScaleFactor = 1.3
    local isCollapsed = IsRadarVisible.value
    return {
      size = [offsetScaleFactor * radarWidth, offsetScaleFactor * radarHeight]
      children = [
        {
...
local B_Scope = function(width, height) {
  local getChildren = function() {
    local children = [
      ...
      B_ScopeAzimuthComponent(width, AimAzimuth, null, null, height, AIM_LINE_WIDTH),
      B_ScopeAzimuthComponent(width, TurretAzimuth, null, null, height, TURRET_LINE_WIDTH),
      B_ScopeSectorComponent(width, TurretAzimuth, TargetRadarDist, TargetRadarAzimuthWidth, height, targetSectorColor)
    ]
    if (IsRadarVisible.value)
      children.append(B_ScopeAzimuthComponent(width, Azimuth, Distance, AzimuthHalfWidth, height))
    if (IsRadar2Visible.value)
      children.append(B_ScopeAzimuthComponent(width, Azimuth2, Distance2, AzimuthHalfWidth2, height))
...
local B_ScopeHalfCircleMarkers = function(radarWidth, radarHeight, is_mfd) {
  local offsetScaleFactor = 1.3
  local hiddenText = (!is_mfd && !IsRadarVisible.value && !IndicationForCollapsedRadar.value)
  if (!hiddenText){
    local scanRangeX = (!is_mfd && IsRadarVisible.value) ? 0.5 : 1.25
    local scanRangeY = (!is_mfd && IsRadarVisible.value) ? 0.5 : 0.75
    local scanYaw = (!is_mfd && IsRadarVisible.value) ? 0.5 : 0.4
    local scanPitch = (!is_mfd && IsRadarVisible.value) ? 0.5 : 0.75
    local fontSize = (!is_mfd && IsRadarVisible.value) ? styleText.fontSize : hdpx(13)
    return {
      size = [offsetScaleFactor * radarWidth, offsetScaleFactor * radarHeight]
      children = [
        {
          ...
          children = @() styleText.__merge({
            ...
            [
              radarWidth * scanRangeX * (1.0 - sin(AzimuthMax.value)) + hdpx(4),
              radarHeight * scanRangeY * (1.0 - cos(AzimuthMax.value)) - hdpx(4)
            ]
            key = $"1{is_mfd}"
            hplace = ALIGN_RIGHT
            watch = [ HasAzimuthScale, ScanAzimuthMin, ScanAzimuthMax,
        ...
        @() styleText.__merge({
          ...
          [
            radarWidth * scanYaw * (1.0 + sin(AzimuthMax.value)) + hdpx(4),
            radarWidth * scanPitch * (1.0 - cos(AzimuthMax.value)) - hdpx(4)
          ]
          watch = [ HasDistanceScale, DistanceMax, DistanceScalesMax ]
          text = HasDistanceScale.value ?
            ::cross_call.measureTypes.DISTANCE.getMeasureUnitsText(DistanceMax.value * 1000.0) +
            (DistanceScalesMax.value > 1 ? "*" : " ") : ""
            fontSize = fontSize
        })
        styleText.__merge({
...
local function B_ScopeHalf(width, height, pos, is_mfd) {
  local getChildren = function() {
    local children = [
      ...
      B_ScopeSectorComponent(width, null, TargetRadarDist, TargetRadarAzimuthWidth, height, targetSectorColor),
      {
        size = [width, height]
        rendObj = ROBJ_RADAR_GROUND_REFLECTIONS
        isSquare = false
    ...
    if (IsRadar2Visible.value)
      children.append(B_ScopeAzimuthComponent(width, Azimuth2, Distance2, AzimuthHalfWidth2, height))
    if (IsAamLaunchZoneVisible.value && HasDistanceScale.value)
      children.append(B_ScopeHalfLaunchRangeComponent(width, height, AzimuthMin, AzimuthMax,
                                                      AamLaunchZoneDistMin, AamLaunchZoneDistMax))
    children.append(targetsComponent(width, height, createTargetOnRadarPolar))
    return children
  }
  ...
  return @() {
    ...
    children = [
      ...
      B_ScopeHalfCircleMarkers(width, height, is_mfd)
    ]
  }
}
...
local C_Scope = function(width, height) {
  local getChildren = function() {
    ...
    if (IsRadarVisible.value)
      children.append(C_ScopeSquareAzimuthComponent(width, height, Azimuth, Elevation, AzimuthHalfWidth, ElevationHalfWidth))
    if (IsRadar2Visible.value)
      children.append(C_ScopeSquareAzimuthComponent(width, height, Azimuth2, Elevation2, AzimuthHalfWidth2, ElevationHalfWidth2))
...
local function radarMfdBackground() {
  local backSize = [radarPosSize.w / RadarScale.value,
    radarPosSize.h / RadarScale.value]
  local backPos = [radarPosSize.x - (1.0 - RadarScale.value) * 0.5 * backSize[0],
   radarPosSize.y - (1.0 - RadarScale.value) * 0.5 * backSize[1]]
  return {
    pos = backPos
    size = backSize
    rendObj = ROBJ_SOLID
    lineWidth = radarPosSize.h
...
local Root = function(for_mfd, radarPosX = sh(8), radarPosY = sh(32), radarSize = sh(28), isAir = false, radar_color = greenColor) {
  ...
  local radarHudVisibleChildren = [
    ...
    forestallComponent
    forestallTargetLine
    radar(radarPosX, radarPosY, radarSize, isAir)
