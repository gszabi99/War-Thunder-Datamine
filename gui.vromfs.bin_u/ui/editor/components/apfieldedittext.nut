...
local getCompVal = @(eid, obj, path, comp_name) path!=null ? getValFromObj(obj, path) : ::ecs.obsolete_dbg_get_comp_val(eid, comp_name)
...
local function fieldEditText(params={}){
  ...
  local function setVal(val) {
    if (path != null) {
      ...
      local ok = false
      try {
        local onChangeLocal = onChange ?? (@() ::ecs.obsolete_dbg_set_comp_val(eid, comp_name, val) ?? true)
        ok = onChangeLocal()
      }
      catch (e) {
        ok = false
